...
namespace COMETwebapp.Tests.Components.ReferenceData.ParameterTypes
{
    ...
    public class DependentParameterTypeTableTestFixture
    {
        ...
        public void VerifyAddButtonFollowsCreatePermission()
        {
            var permissionService = new Mock<IPermissionService>();
            permissionService.Setup(x => x.CanWrite(It.IsAny<Thing>())).Returns(true);
            permissionService.Setup(x => x.CanWrite(ClassKind.DependentParameterTypeAssignment, It.IsAny<Thing>())).Returns(false);
            var session = new Mock<ISession>();
            session.Setup(x => x.PermissionService).Returns(permissionService.Object);
            this.sessionService.Setup(x => x.Session).Returns(session.Object);

            this.renderer.Render();
            var addButton = this.renderer.FindComponents<DxButton>().First(x => x.Instance.Id == "addDependentParameterTypeButton");
            Assert.That(addButton.Instance.Enabled, Is.False, "the user may not add a dependent parameter type without the create permission on the parent thing");

            permissionService.Setup(x => x.CanWrite(ClassKind.DependentParameterTypeAssignment, It.IsAny<Thing>())).Returns(true);
            this.renderer.Render();
            addButton = this.renderer.FindComponents<DxButton>().First(x => x.Instance.Id == "addDependentParameterTypeButton");
