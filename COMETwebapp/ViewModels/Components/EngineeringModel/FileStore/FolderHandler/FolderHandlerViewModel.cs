...
namespace COMETwebapp.ViewModels.Components.EngineeringModel.FileStore.FolderHandler
{
    ...
    public class FolderHandlerViewModel : SingleThingApplicationBaseViewModel<Folder>, IFolderHandlerViewModel
    {
        ...
        /// <summary>
        /// Moves a folder to a target folder. Silently refuses the move when the current user lacks write
        ...
        {
            if (!this.SessionService.Session.PermissionService.CanWrite(ClassKind.Folder, targetFolder))
            {
                return;
            }
