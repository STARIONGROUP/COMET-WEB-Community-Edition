// --------------------------------------------------------------------------------------------------------------------
//  <copyright file="DefaultValues.razor.cs" company="Starion Group S.A.">
...
namespace COMETwebapp.Components.ModelDashboard.ParameterValues
{
	...
	/// <summary>
	/// <see cref="HaveChartData" /> where the value is "-"
	/// </summary>
	public partial class DefaultValues
	{
		...
		{
			return criteria.Equals("Missing Values", StringComparison.InvariantCultureIgnoreCase)
				? this.ValueSets.Count(d => d.Owner.ShortName.Equals(domain) && d.Published.All(v => v == "-"))
				: this.ValueSets.Count(d => d.Owner.ShortName.Equals(domain) && d.Published.Any(v => v != "-"));
		}
	}
}
